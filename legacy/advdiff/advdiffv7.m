...
% Apparently eps is a reserved word, so full name
% TODO find out what epsilon should be.
epsilon = 1;
...
%  A(1,1) = -(abs((n*dt-t0)) < dt/2)/gam;
  ...
  v = inv(I - theta*D*dt*A) * (vold + (1-theta)*D*vzz + qdot);
