...
for n = 1:N,
  ...
  A(1,1) = -gam*(abs((n*dt-t0)) < abs(dt/2));
  vzz = A*v;
  ...
  % (vnp1 - vn)/dt = D(v_{i-1} - 2v_i + v_{i+1})/dt^2 = DA \vec{v}
  
  % Note that this is what we had previously:
  % v = vold+D*vzz+dt*kap*ha*qdot;
  % TODO which one is correct.
  v = vold+dt*D*vzz+dt*kap*ha*qdot;
